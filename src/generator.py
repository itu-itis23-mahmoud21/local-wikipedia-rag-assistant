...
class OllamaAnswerGenerator:
    ...
    def build_prompt(self, query: str, context: str) -> str:
        ...
        return (
            ...
            "unless X is directly part of the user's question. For comparison "
            "or multi-entity questions, compare only using available context "
            "and the entity labels in each source. Prefer this structure: "
            "Entity A:, Entity B:, Comparison:. Keep facts separated by entity, "
            "describe each compared entity separately first, and do not transfer "
            "facts from one entity to another. If a fact appears only in one "
            "entity's source/context, state it only under that entity and do not "
            "assign it to the other entity. If a comparison detail is missing "
            "for one entity, say that the retrieved context does not provide "
            "that detail for that entity. Do not invent balanced statistics. "
